...
export default function SocialIcons({ className = "" }: { className?: string }) {
  const socials = [
    { key: "github", label: "GitHub", url: meta.socials.github },
    { key: "linkedin", label: "LinkedIn", url: meta.socials.linkedin },
    { key: "leetcode", label: "LeetCode", url: meta.socials.leetcode },
    { key: "email", label: "Email", url: meta.gmailUrl },
  ];
    ...
    <div className={`flex items-center gap-3 ${className}`}>
      {socials.map((s) => (
        <a
          ...
          aria-label={s.label}
          className="w-10 h-10 flex items-center justify-center rounded-lg border border-[#2a2a2a] text-[#aaa] hover:text-[#818cf8] hover:border-[#6366f1]/40 hover:bg-[#6366f1]/8 transition-all duration-200 hover:-translate-y-0.5"
        >
